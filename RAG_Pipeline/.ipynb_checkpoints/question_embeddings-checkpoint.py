...
def main(path):
    ...
    for i in range(len(egs)):
        video_code = egs[i]
        if not Path(base_dir + path + video_code).exists():
            continue
        ...
        with open(base_dir + path + video_code + "/answers.pkl", "wb") as file:
            pickle.dump(embeddings, file)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="parser for step3")
    parser.add_argument('--path', type=str, help='path in question')
