...
# select frames
def select_frames(folder, divlam, n, preprocess, model):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    img_list = natsort.natsorted(glob.glob(f"{folder}/*.jpg"))
    img_feats = []

    img_set = loading_img(img_list, preprocess)
    img_loader = DataLoader(img_set, batch_size=64, shuffle=False, num_workers=1)
    ...
    with torch.no_grad():
        featuremap = model(img_feats)
        frame_num = featuremap.shape[0]

        dist_list = []
        for img_feat in featuremap: 
            dist_list.append(torch.mean(torch.sqrt((featuremap-img_feat)**2), dim=-1))
